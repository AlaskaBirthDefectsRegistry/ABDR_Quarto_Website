---
title: "Total Anomalous Pulmonary Venous Return (TAPVR) 2007-`r as.numeric(format(Sys.time(), '%Y')) - 3`"
...
Total Anomalous Pulmonary Venous Return (TAPVR) is a birth defect of the heart that occurs when oxygen-rich blood from the lungs does not return normally to the left atrium, but rather to the right atrium. Babies with TAPVR usually have a hole between the right and left atrium (an atrial septal defect) that allows oxygenated and deoxygenated blood to mix throughout the heart. [[1]](#references) Mixed blood on the left side of the heart is then pumped to the rest of the body. [[2]](#references) There are four types of TAPVR – 1) Supracardiac, where the pulmonary veins connect above the heart, Cardiac, where the pulmonary veins connect behind the heart, Infracardiac, where the pulmonary veins connect below the heart, and Mixed, which is a combination of any of the previously listed connections. [[3]](#references) TAPVR is usually diagnosed after birth by pulse oximetry, chest x-ray, and echocardiography, but can be diagnosed during pregnancy. [[1]](#references) Some of the signs and symptoms of TAPVR are cyanosis (blue skin tone) respiratory distress, and heart murmur. [[4]](#references) Surgery is the most common treatment for this condition. Surgical repair includes connecting the pulmonary veins to the left atrium, closing the atrial septal defect and any other abnormal connections to the heart. [[1]](#references) Because an infant with total anomalous pulmonary venous return may need these procedures soon after birth, this defect is considered a critical congenital heart defect.

<br>

<iframe width="812" height="456.75" src="https://www.youtube.com/embed/RMIKq92dfHE" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen data-external = "1"></iframe>
...
### Prevalence   
Total Anomalous Pulmonary Venous Return occurs in about **1.28 (95% CI 1.18–1.38) out of every 10,000 live births in the United States.** This results in about 504 babies diagnosed with Total Anomalous Pulmonary Venous Return nationally each year.[[1]](#references), [[5]](#references)
...
# Create a label for the condition to be used
  condition <- ("Total Anomalous Pulmonary Venous Return")
...
# Loading the TAPVC
ev2 <- readRDS("R:/ABDR/Analysis_New/ABDR_CASECONF/cond_reports/Published_reports/abdr_website/images/ev2_num_TAPVC.rds") #
...
tmp <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\tmp_TAPVC.rds")
tmp[[1]]
```  
...
library(plotly)
 ...
 trend <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\trend_nma_TAPVC.rds")
  
 trend_table <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\trend_table_nma_TAPVC.rds")
...
#| echo: false
 ...
 p_val <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\trend_p_table_TAPVC.rds")
...
geo_plot <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\sf_plot_map_TAPVC.rds")
 ggplotly(geo_plot) %>%
  layout(xaxis = list(autorange = TRUE),
         yaxis = list(autorange = TRUE))

geo_tbl <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\geo_tbl_TAPVC.rds")
...
t6 <- readRDS("R:/ABDR/Analysis_New/ABDR_CASECONF/cond_reports/Published_reports/abdr_website/images/t6_TAPVC.Rds")
t7 <- readRDS("R:/ABDR/Analysis_New/ABDR_CASECONF/cond_reports/Published_reports/abdr_website/images/t7_TAPVC.Rds")
...
plot <- readRDS("R:/ABDR/Analysis_New/ABDR_CASECONF/cond_reports/Published_reports/abdr_website/images/accomp_diag_TAPVC.Rds")
...
## References
[[1]]( https://www.cdc.gov/ncbddd/heartdefects/tapvr.html) Congenital Heart Defects - Facts about TAVPR | CDC. (2019). Retrieved 17 March 2021, from https://www.cdc.gov/ncbddd/heartdefects/tapvr.html

[[2]]( https://www.mayoclinic.org/diseases-conditions/total-anomalous-pulmonary-venous-return/cdc-20385613) Total anomalous pulmonary venous return - Overview - Mayo Clinic. (2021). Retrieved 17 March 2021, from https://www.mayoclinic.org/diseases-conditions/total-anomalous-pulmonary-venous-return/cdc-20385613

[[3]]( https://www.childrenshospital.org/conditions-and-treatments/conditions/t/total-anomalous-pulmonary-venous-return?utm_campaign=FY21HeartFetal&utm_medium=cpc&utm_source=google&utm_content=tapvr&gclid=EAIaIQobChMI26LUhZjM7gIVTT6tBh3n1wl_EAAYASAAEgJmxvD_BwE) Total Anomalous Pulmonary Venous Return (TAPVR) | Boston Children's Hospital. (2021). Retrieved 17 March 2021, from https://www.childrenshospital.org/conditions-and-treatments/conditions/t/total-anomalous-pulmonary-venous-return?utm_campaign=FY21HeartFetal&utm_medium=cpc&utm_source=google&utm_content=tapvr&gclid=EAIaIQobChMI26LUhZjM7gIVTT6tBh3n1wl_EAAYASAAEgJmxvD_BwE

[[4]](  https://www.chop.edu/conditions-diseases/total-anomalous-pulmonary-venous-return-tapvr?gclid=EAIaIQobChMI28qEgZnM7gIVkgh9Ch2DHwn0EAAYAiAAEgIR9vD_BwE) Total Anomalous Pulmonary Venous Return (TAPVR) | Children's Hospital of Philadelphia. (2021). Retrieved 17 March 2021, from https://www.chop.edu/conditions-diseases/total-anomalous-pulmonary-venous-return-tapvr?gclid=EAIaIQobChMI28qEgZnM7gIVkgh9Ch2DHwn0EAAYAiAAEgIR9vD_BwE
...
## Resources 
[Centers for Disease Control and Prevention](https://www.cdc.gov/ncbddd/heartdefects/tapvr.html)

[Seattle Children's:Transposition of the Great Arteries]((https://www.seattlechildrens.org/conditions/total-anomalous/))  
...
*Code source:* ```R:\ABDR\Website\ABDR_Quarto_Website\TAPVR_tar.qmd```   
