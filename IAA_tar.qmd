---
title: "Interrupted Aortic Arch 2007-`r as.numeric(format(Sys.time(), '%Y')) - 3`"
...
Interrupted aortic arch (IAA) is a birth defect of the heart defined by the abnormal development of the aortic arch system during the fifth to seventh week of fetal development. Interrupted aortic arch is characterized by absence or discontinuation of a portion of the aortic arch, the main vessel that carries oxygen-rich blood away from the heart to the other organs of the body. It is most always associated with a large ventricular septal defect, or a hole in the wall between the two lower chambers of the heart. [[1]](#references) IAA can be diagnosed by prenatal ultrasound but is frequently missed. Infants can present clinically in the early neonatal period with signs and symptoms of heart failure (respiratory distress, poor feeding) and systemic hypoperfusion (decreased urine output and poor pulses, especially in the lower extremities) when the ductus arteriosus closes. [[1]](#references) Because this condition is associated with low oxygen levels soon after birth, newborn screening with pulse oximetry can lead to earlier diagnosis. This condition can be associated with genetic risk factors such as 22q11 deletion. Initial treatment for IAA is focused on supporting the infant until surgical procedures to create a continuous aortic arch and closure of the ventricular septal defect can be performed. [[2]](#references) Because an infant with interrupted aortic arch needs these procedures soon after birth, this defect is considered a critical congenital heart defect.
Epidemiology


<br>

<iframe width="812" height="456.75" src="https://www.youtube.com/embed/XM3m-PjhH24" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen data-external = "1"></iframe>
...
### Prevalence   
Interrupted aortic arch occurs in about **0.62 (95% CI 0.55–0.70) in every 10,000 live births in the United States.** [[3]](#references)
...
# Create a label for the condition to be used
  condition <- ("Interrupted aortic arch")                                            
...
# Loading the IAA
ev2 <- readRDS("R:/ABDR/Analysis_New/ABDR_CASECONF/cond_reports/Published_reports/abdr_website/images/ev2_num_IAA.rds") #
...
tmp <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\tmp_IAA.rds")
tmp[[1]]
```  
...
library(plotly)
 ...
 trend <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\trend_nma_IAA.rds")
  
 trend_table <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\trend_table_nma_IAA.rds")
...
p_val <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\trend_p_table_IAA.rds")
...
geo_plot <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\sf_plot_map_IAA.rds")
 ggplotly(geo_plot) %>%
  layout(xaxis = list(autorange = TRUE),
         yaxis = list(autorange = TRUE))

geo_tbl <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\geo_tbl_IAA.rds")
...
t6 <- readRDS("R:/ABDR/Analysis_New/ABDR_CASECONF/cond_reports/Published_reports/abdr_website/images/t6_IAA.Rds")
t7 <- readRDS("R:/ABDR/Analysis_New/ABDR_CASECONF/cond_reports/Published_reports/abdr_website/images/t7_IAA.Rds")
...
plot <- readRDS("R:/ABDR/Analysis_New/ABDR_CASECONF/cond_reports/Published_reports/abdr_website/images/accomp_diag_IAA.Rds")
...
## References
[[1]](https://www.chop.edu/conditions-diseases/interruption-aortic-arch) Interruption of the Aortic Arch | Children's Hospital of Philadelphia. (2021). Retrieved 19 February 2021, from https://www.chop.edu/conditions-diseases/interruption-aortic-arch

[[2]](https://www.cincinnatichildrens.org/health/i/iaa) Cincinnati Children’s - Interrupted Aortic Arch | Types, Treatment & Repair . (2021). Retrieved 19 February 2021, from https://www.cincinnatichildrens.org/health/i/iaa
...
## Resources 
[Centers for Disease Control and Prevention](https://www.cdc.gov/ncbddd/heartdefects/cchd-facts.html)

[Seattle Children's:Transposition of the Great Arteries](https://www.seattlechildrens.org/conditions/interrupted-aortic-arch/)  
...
## Suggested Citation
State of Alaska Department of Health, Division of Public Health, Section of Women's, Children's, and Family Health. Alaska Birth Defects Registry Condition Report: Interrupted Aortic Arch, Alaska, 2007-`r as.numeric(format(Sys.time(), '%Y')) - 3`. Updated `r format(Sys.time(), '%B %d, %Y')`. Available at: http://rpubs.com/AK_ABDR/IAA.
...
*Code source:* ```R:\ABDR\Website\ABDR_Quarto_Website\IAA_tar.qmd```   
