---
title: "Alaska Birth Defect Registy - 5-Year Reported Prevalence and Defect Estimates"
subtitle: "Birth Years `r as.numeric(format(Sys.time(), '%Y')) - 8`-`r as.numeric(format(Sys.time(), '%Y')) - 3`"
...
**5-Year Report and Estimated Defect Prevalence Estimates (per 10,000 live births)**  
```{r} 
#| label: chunk-label-1  
#| warning: false 
#| message: FALSE 
...
#### CL ####
cl15 <- ev2_cl %>% filter(yrs %in% 2018:2022) %>%                               # #
...
#### CP
cp15 <- ev2_cp %>% filter(yrs %in% 2018:2022) %>%                               # #
  select(-c('Prevalence (95% CI)', yrs)) %>%
  mutate_all(.funs = as.numeric) %>%
  summarise_all(.funs = sum)
cp15b <- cp15/5
...
#### CLP ####
clp15 <- ev2_clp %>% filter(yrs %in% 2018:2022) %>%                             # #
  select(-c('Prevalence (95% CI)', yrs)) %>%
  mutate_all(.funs = as.numeric) %>%
  summarise_all(.funs = sum)
clp15b <- clp15/5
