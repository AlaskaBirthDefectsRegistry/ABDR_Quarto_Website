...
trend1[,2:17]<-round(trend1[,2:17],1)                                           # # add one to all last column numbers
trend1[,18]<-round(trend1[,18],4)                                               # #
trend1[,2:18]<-format(trend1[,2:18],drop0trailing = FALSE,trim = T)             # #
trend1[,2:18][trend1[,2:18]=="NA"|trend1[,2:18]=="NaN"]<- 0 # used to be "-" not zero   # # #
...
tidytrend <- trend2 %>%
  rownames_to_column() %>%
  select(-`p-value`) %>%
  pivot_longer( names_to = "year", values_to = "reports", `2007`:`2022`) %>%    #
...
demo_tbl <- datatable(trend4,
          ...
          options = list(rowGroup = list(dataSrc = 19),                         # Add one to dataSrc to keep the right grouping number
                         pageLength = 47, 
                         dom = 't',
...
*Code source:* ``R:\ABDR\Website\ABDR_Quarto_Website\trend_currnet.qmd``` 
