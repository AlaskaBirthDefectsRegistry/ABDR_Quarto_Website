---
title: "Limb Deficiencies 2007-`r as.numeric(format(Sys.time(), '%Y')) - 3`"
...
Limb deficiencies are reductions or defects of the upper or lower limbs. These occur when a fetus's arm, hand (upper limb), leg, or foot (lower limb) does not form correctly during pregnancy.[[1]](#references) There isn't a single cause of congenital limb deficiencies, and often the cause is unknown. Risk factors that may increase the likelihood of limb deficiencies include genetic abnormalities, growth restriction, mechanical forces that affect the baby in the uterus during development, and in-utero exposure to chemicals, medications, or some viruses.[[[2]][[3]](#references)  An estimated 1 in every 1,900 babies (5.27 per 10,000) are born with a limb deficiency in the United States.[[4]](#references)  Treatment for limb deficiencies depends on many factors, including the child's age, the family's wishes, and the nature of the deficiency.[[3]](#references)  Children with limb deficiencies may experience some difficulties due to their affected limb(s) but can also live a healthy and productive life. 
...
### Prevalence   
Limb Deficiencies occurs in **5.27 (95% CI 5.07, 5.47) out of every 10,000 live births in the United States.** This results in about 1,900 babies diagnosed with the condition nationally each year.[[1]](#references), [[4]](#references)
...
# Loading the LIMB
ev2 <- readRDS("R:/ABDR/Analysis_New/ABDR_CASECONF/cond_reports/Published_reports/abdr_website/images/ev2_num_LIMB.rds") #
...
tmp <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\tmp_LIMB.rds")
tmp[[1]]
```  
</center>
...
library(plotly)
 ...
 trend <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\trend_nma_LIMB.rds")
  
 trend_table <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\trend_table_nma_LIMB.rds")
...
#| echo: false
 ...
 p_val <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\trend_p_table_LIMB.rds")
...
geo_plot <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\sf_plot_map_LIMB.rds")
 ggplotly(geo_plot) %>%
  layout(xaxis = list(autorange = TRUE),
         yaxis = list(autorange = TRUE))

geo_tbl <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\geo_tbl_LIMB.rds")
...
t6 <- readRDS("R:/ABDR/Analysis_New/ABDR_CASECONF/cond_reports/Published_reports/abdr_website/images/t6_LIMB.Rds")
t7 <- readRDS("R:/ABDR/Analysis_New/ABDR_CASECONF/cond_reports/Published_reports/abdr_website/images/t7_LIMB.Rds")
...
plot <- readRDS("R:/ABDR/Analysis_New/ABDR_CASECONF/cond_reports/Published_reports/abdr_website/images/accomp_diag_LIMB.Rds")
...
## References
[[1]](https://www.cdc.gov/ncbddd/birthdefects/ul-limbreductiondefects.html) Specific Birth Defects- Facts about Upper and Lower Limb Reduction Defects (CDC), from https://www.cdc.gov/ncbddd/birthdefects/ul-limbreductiondefects.html (2023) [accessed January, 02 2023]  

[[2]](https://www.chop.edu/conditions-diseases/congenital-limb-defects) Children's Hospital of Philadelphia, Congenital Limb Defects. https://www.chop.edu/conditions-diseases/congenital-limb-defects(2024). Retrieved 08 January 2024, from  2015 [accessed 03.06.2017]

[[3]](https://www.shrinerschildrens.org/en/pediatric-care/limb-deficiencies) Shriner’s Children. Limb Deficiencies (2024). Retrieved 08 January 2024, from https://www.shrinerschildrens.org/en/pediatric-care/limb-deficiencies 
...
## Resources 
[Centers for Disease Control and Prevention](https://www.cdc.gov/ncbddd/birthdefects/ul-limbreductiondefects.html)

[Shriners Children's](https://www.shrinerschildrens.org/en/pediatric-care/limb-deficiencies)  
...
*Code source:* ```R:\ABDR\Website\ABDR_Quarto_Website\limb_tar.qmd```   
