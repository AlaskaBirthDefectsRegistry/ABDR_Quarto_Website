---
title: "Pulmonary Atresia 2007-`r as.numeric(format(Sys.time(), '%Y')) - 3`"
...
Pulmonary atresia is a birth defect of the heart where the pulmonary valve fails to form. Due to this defect, blood from the right ventricle cannot get directly into the lungs from the pulmonary artery as in a normal heart. [[1]](#references) In newborns with pulmonary atresia, the blood bypasses the unformed pulmonary valve through openings between the upper chambers of the heart (patent foramen ovale) and from the aorta to the pulmonary artery (ductus arteriosus) that ordinarily exist during gestation. These openings normally close shortly after birth but are critical for the survival of patients with pulmonary atresia. [[1]](#references) There are two types of pulmonary atresia - 1) pulmonary atresia with a ventricular septal defect, or 2) pulmonary atresia without a ventricular septal defect (intact septum). In pulmonary atresia without a ventricular septal defect, the right ventricle receives little blood flow, causing it to be underdeveloped at birth. [[3]](#references) With pulmonary atresia with ventricular septal defect, the defect allows blood to flow to the right ventricle, which simulates more normal development of the ventricle. [[1]](#references) Pulmonary atresia can be diagnosed before birth by ultrasound, or soon after birth via echocardiogram. [[2]](#references) Typical treatments for this condition are medication (prostaglandin) to maintain the ductus arteriosus, balloon valvotomy to open the pulmonary valve, ductal stent placement to keep this connection open to allow blood flow to the lungs, or a systemic-to-pulmonary artery shunt that connects one of the arteries to the pulmonary artery [[4]](#references) Infants with pulmonary atresia often need surgery or other procedures soon after birth, therefore, this defect is considered a critical congenital heart defect (CCHD). Common signs and symptoms for this defect are cyanosis (blue skin tone) which can be detected by pulse oximetry, shortness of breath, difficulty feeding, and heart murmur.  [[2]](#references)

 ![](R:/ABDR/Analysis_New/ABDR_CASECONF/cond_reports/Published_reports/pul_atr.png "Pulmonary atresia birth defect diagram")
...
### Prevalence   
Pulmonary atresia occurs in about **1.41 (95% CI 1.30–1.52) out of every 10,000 live births in the United States.** This results in about 550 babies diagnosed with pulmonary atresia nationally each year.[[5]](#references)
...
# Loading the PVAS
ev2 <- readRDS("R:/ABDR/Analysis_New/ABDR_CASECONF/cond_reports/Published_reports/abdr_website/images/ev2_num_PVAS.rds") #
...
#| message: false 
#| echo: false
tmp <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\tmp_PVAS.rds")
tmp[[1]]
```  
...
library(plotly)
 ...
 trend <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\trend_nma_PVAS.rds")
  
 trend_table <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\trend_table_nma_PVAS.rds")
...
#| message: false 
#| echo: false
 p_val <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\trend_p_table_PVAS.rds")
...
#| message: false 
#| echo: false 
geo_plot <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\sf_plot_map_PVAS.rds")
 ggplotly(geo_plot) %>%
  layout(xaxis = list(autorange = TRUE),
         yaxis = list(autorange = TRUE))

geo_tbl <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\geo_tbl_PVAS.rds")
...
t6 <- readRDS("R:/ABDR/Analysis_New/ABDR_CASECONF/cond_reports/Published_reports/abdr_website/images/t6_PVAS.Rds")
t7 <- readRDS("R:/ABDR/Analysis_New/ABDR_CASECONF/cond_reports/Published_reports/abdr_website/images/t7_PVAS.Rds")
...
plot <- readRDS("R:/ABDR/Analysis_New/ABDR_CASECONF/cond_reports/Published_reports/abdr_website/images/accomp_diag_PVAS.Rds")
...
## References
[[1]]( https://www.cdc.gov/ncbddd/heartdefects/pulmonaryatresia.html) Congenital Heart Defects - Facts about Pulmonary Atresia | CDC. (2019). Retrieved 20 March 2021, from https://www.cdc.gov/ncbddd/heartdefects/pulmonaryatresia.html

[[2]]( https://www.chop.edu/conditions-diseases/pulmonary-atresia) Pulmonary Atresia | Children's Hospital of Philadelphia. (2021). Retrieved 20 March 2021, from https://www.chop.edu/conditions-diseases/pulmonary-atresia

[[3]]( https://www.childrenshospital.org/conditions-and-treatments/conditions/p/pulmonary-atresia) Pulmonary Atresia | Boston Children's Hospital. (2021). Retrieved 20 March 2021, from https://www.childrenshospital.org/conditions-and-treatments/conditions/p/pulmonary-atresia

[[4]](  https://www.mayoclinic.org/diseases-conditions/pulmonary-atresia-intact-ventricular-septum/cdc-20396714) Pulmonary atresia with intact ventricular septum - Overview - Mayo Clinic. (2021). Retrieved 20 March 2021, from https://www.mayoclinic.org/diseases-conditions/pulmonary-atresia-intact-ventricular-septum/cdc-20396714
...
## Resources 
[Centers for Disease Control and Prevention](https://www.cdc.gov/ncbddd/heartdefects/pulmonaryatresia.html)

[Seattle Children's:Transposition of the Great Arteries]((https://www.seattlechildrens.org/conditions/pulmonary-atresia/))  
...
## Suggested Citation
State of Alaska Department of Health, Division of Public Health, Section of Women's, Children's, and Family Health. Alaska Birth Defects Registry Condition Report: Pulmonary Atresia, Alaska, 2007-`r as.numeric(format(Sys.time(), '%Y')) - 3`. Updated `r format(Sys.time(), '%B %d, %Y')`. Available at: http://rpubs.com/AK_ABDR/pulmonary_atresia
...
*Code source:* ```R:\ABDR\Website\ABDR_Quarto_Website\Pulmonary_Atresia_tar.qmd```   
