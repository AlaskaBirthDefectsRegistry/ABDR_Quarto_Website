---
title: "Cleft Lip 2007-`r as.numeric(format(Sys.time(), '%Y')) - 3`"
...
---
## Background
A cleft lip is congenital condition when a baby's lip does not form properly during early pregnancy [[1]](#references). Cleft lip occurs when the lip does not come together during development, separating the lip either on one side (unilateral) or on both sides (bilateral) [[2]](#references). Cleft palate occurs when the roof of the mouth fails to join together resulting in an opening in either the front of the palate, the back of the palate, or both [[1]](#references). The lip and palate develop separately during pregnancy, therefore, a baby can have either cleft lip, cleft palate, or both cleft lip and cleft palate [[2]](#references). Orofacial clefts are one of the most common birth defects in the United States [[2, 3]](#references).
While the exact cause for most cases of orofacial clefts is unknown. It is believed that a combination of environmental factors, mothers behaviors during early pregnancy (what she eats/drinks and medicine consumed), and genetic factors may contribute to orofacial clefts [[1, 2]](#references). 
...
# Loading the CL
ev2 <- readRDS("R:/ABDR/Analysis_New/ABDR_CASECONF/cond_reports/Published_reports/abdr_website/images/ev2_num_CL.rds") #

```

### Prevalence 
According to the 2019 annual report released by the National Birth Defects Prevention Network (NBDPN), the prevalence of cleft lip from 2010-2014 was was **3.56 (95% CI 3.39–3.74) per 10,000 live births.**[[3]](#references)

In Alaska, during `r datYRS[1]`, the prevalence of cleft lip was **`r ev2[max(row.names(ev2)),4]` per 10,000 live births**. 
<center>  
```{r} 
#| warning: false 
#| message: false 
...
tmp <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\tmp_CL.rds")
tmp[[1]]
```  
</center>
...
library(plotly)
 ...
 trend <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\trend_nma_CL.rds")
  
 trend_table <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\trend_table_nma_CL.rds")
...
#| echo: false
 ...
 p_val <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\trend_p_table_CL.rds")
...
geo_plot <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\sf_plot_map_CL.rds")
 ggplotly(geo_plot) %>%
  layout(xaxis = list(autorange = TRUE),
         yaxis = list(autorange = TRUE))

geo_tbl <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\geo_tbl_CL.rds")
...
t6 <- readRDS("R:/ABDR/Analysis_New/ABDR_CASECONF/cond_reports/Published_reports/abdr_website/images/t6_CL.Rds")
t7 <- readRDS("R:/ABDR/Analysis_New/ABDR_CASECONF/cond_reports/Published_reports/abdr_website/images/t7_CL.Rds")
...
plot <- readRDS("R:/ABDR/Analysis_New/ABDR_CASECONF/cond_reports/Published_reports/abdr_website/images/accomp_diag_CL.Rds")
...
*Updated: ```R:\ABDR\Website\ABDR_Quarto_Website\cleft_lip_tar.qmd```   
