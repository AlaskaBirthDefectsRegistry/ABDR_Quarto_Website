---
title: "Congenital Diaphragmatic Hernia (CDH) 2007-`r as.numeric(format(Sys.time(), '%Y')) - 3`"
...
---
## Background  
Congenital diaphragmatic hernia (CDH) is a birth defect where a hole in the diaphragm allows for the passage of organs in the abdomen, such as the intestines, stomach, and liver, upwards into an infant's chest.[[1]](#references) The diaphragm is the large muscle that separates the chest from abdomen and when an organ in the abdomen pushes through the hole in the diaphragm, it is considered a hernia. The presence of these organs in the chest can cause breathing difficulties and prevent the infant's lungs from developing completely.[[1]](#references) CDHs can be diagnosed before the infant is born through an ultrasound. If the defect is not found before birth, doctors may diagnose the condition through a chest x-ray and presence of symptoms, like difficulty breathing after the baby is born.[[2]](#references) Treatment requires surgery soon after birth to repair the hernia and continued breathing assistance is often necessary to help the lungs recover and fully develop.   

The exact causes of CDH are unknown, although increasing evidence points to genetic causation. Some research has shown that about 15-20% of all CDH cases are caused by abnormalities in the infant's genes, but the causes of the remaining 80-85% of CDH cases are unknown.[[3]](#references) 
...
### Prevalence 
Congenital diaphragmatic hernia occurs in **2.79 (95% CI 2.64–2.94) out of every 10,000 live births in the United States.** [[1]](#references)  
...
#Create a label for the condition to be used
  UCcondition<-("Congenital diaphragmatic hernia")
  LCcondition<-("congenital diaphragmatic hernia")                                              #
...
# Loading the ANEN
ev2 <- readRDS("R:/ABDR/Analysis_New/ABDR_CASECONF/cond_reports/Published_reports/abdr_website/images/ev2_num_DI.rds") #


```
  
   
In Alaska, during `r datYRS[1]`, the prevalence of `r LCcondition[1]` was **`r ev2[max(row.names(ev2)),4]` per 10,000 live births**. 
<center>
```{r} 
#| warning: false 
#| message: false 
...
tmp <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\tmp_DI.rds")
tmp[[1]]
```  


### Trend 
Prevalence per 10,000 births of `r LCcondition[1]` during `r datYRS[1]` by three-year moving averages, with 95% confidence interval band and Poisson estimated fitted line.
<center>
```{r} 
#| warning: false 
#| message: false 
...
library(plotly)
 ...
 trend <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\trend_nma_DI.rds")
  
 trend_table <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\trend_table_nma_DI.rds")
...
The p-value test for trend detected a significant difference from a slope of zero. Visual inspection of the trend indicates a decrease in the number of live births with `r LCcondition[1]` during `r datYRS[1]`. See [p-value estimate](#p-value-estimate)
<center>
```{r} 
#| warning: false 
#| message: false 
#| echo: false
 ...
 p_val <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\trend_p_table_DI.rds")
...
Distribution of `r LCcondition[1]` in Alaska by Public Health Region of maternal residence at the time of birth. [A description of regional breakdowns can be found here](http://dhss.alaska.gov/dph/InfoCenter/Pages/ia/brfss/geography.aspx). Data suppressed for # of reports  < 6.   
<center>
```{r} 
#| warning: false 
#| message: false 
...
geo_plot <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\sf_plot_map_DI.rds")
 ggplotly(geo_plot) %>%
  layout(xaxis = list(autorange = TRUE),
         yaxis = list(autorange = TRUE))

geo_tbl <- readRDS("R:\\ABDR\\Analysis_New\\ABDR_CASECONF\\cond_reports\\Published_reports\\abdr_website\\images\\geo_tbl_DI.rds")
...
### Demographics
Some subgroups may be more at risk for having a baby with `r LCcondition[1]`. This section provides the descriptive epidemiology of specified maternal, birth, and child characteristics identified from the birth certificate.  
...
t6 <- readRDS("R:/ABDR/Analysis_New/ABDR_CASECONF/cond_reports/Published_reports/abdr_website/images/t6_DI.Rds")
t7 <- readRDS("R:/ABDR/Analysis_New/ABDR_CASECONF/cond_reports/Published_reports/abdr_website/images/t7_DI.Rds")
...
The ten diagnoses most commonalty associated with `r LCcondition[1]`. 
...
## References
[[1]](https://onlinelibrary.wiley.com/doi/abs/10.1002/bdr2.1589) Mai CT, Isenburg JL, Canfield MA, Meyer RE, Correa A, Alverson CJ, Lupo PJ, Riehle‐Colarusso T, Cho SJ, Aggarwal D, Kirby RS. National population‐based estimates for major birth defects, 2010–2014. Birth Defects Research. 2019; 111(18): 1420-1435 

[[2]](https://ojrd.biomedcentral.com/track/pdf/10.1186/1750-1172-7-1?site=ojrd.biomedcentral.com) Tovar JA. Congenital diaphragmatic hernia. Orphanet Journal of Rare Disease. 2012; 7(1)  

[[3]](http://www.ncbi.nlm.nih.gov/books/NBK1359/) Pober BR, Russell MK, Ackerman KG. Congenital diaphragmatic hernia overview. 2006 [Updated 2010 Mar 16]. GeneReviews. Available at: http://www.ncbi.nlm.nih.gov/books/NBK1359/
       

## Resources 
[Centers for Disease Control and Prevention](https://www.cdc.gov/ncbddd/birthdefects/diaphragmatichernia.html#ref)  
[Birth Defects Study to Evaluate Pregnancy exposureS (BD-STEPS)](https://www.cdc.gov/ncbddd/birthdefects/bd-steps.html)  
...
## Suggested Citation
State of Alaska Department of Health, Division of Public Health, Section of Women's, Children's, and Family Health. Alaska Birth Defects Registry Condition Report: Congenital Diaphragmatic Hernia (CDH), Alaska, 2007-2017. Updated `r up_date[1]`. Available at: http://rpubs.com/AK_ABDR/CongenitalDiaphragmaticHernia.
...
*Updated: `r up_date[1]`*  
*Code source:* ```R:\ABDR\Website\ABDR_Quarto_Website\DiaphragmaticHernia.qmd```  
